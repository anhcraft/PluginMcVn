...
<div class="list-plugin-sub">
	...
	<div id="list-plugin-sub-content">
		...
		<div id="list-plugin-sub-info">
			...
			<span><a href="http://pluginmcvn.cf/resource/free.plugin?category='.$aif["category"].'">'.toCategory($aif["category"]).'</a></span>
		</div>
...
if($listPlugin == ""){
	if(1 < ((int) $page)){
		if($category == null){
			header("Location: ". regurl("@p://@d/resource/free.plugin?page=".(((int) $page)-1)));
		} else {
			header("Location: ". regurl("@p://@d/resource/free.plugin?page=".(((int) $page)-1)."&category=".$category));
		}
	} else {
...
if(1 < $rs){
	...
	for($i = 0; $i < $rs; $i++){
		...
		if($i == 0){
			$pagemenu .= '<li '.$pms.'><a href="http://pluginmcvn.cf/resource/free.plugin">'.($i+1).'</a></li>';
			continue;
		}
		$pagemenu .= '<li '.$pms.'><a href="http://pluginmcvn.cf/resource/free.plugin?page='.($i+1).'">'.($i+1).'</a></li>';
	}
...
class Setup {
	var $title = "Plugin MCVN - Plugin Miễn phí";
	var $robots = "{public}";
	var $description = "";
	var $importCSS = array();
	var $importJS = array();
...
<div class="container pluginmcvn-container">
	<h1 id="pluginmcvn-bigtitle" class="padd-big">Plugin Miễn phí</h1>
	<div id="side-area">
		<div id="side-top" class="padd-large">
			<div class="side-top-heading">Danh mục</div>
			<div class="side-top-menu">
				<a href="http://pluginmcvn.cf/resource/free.plugin?page=<?php echo $page; ?>">Toàn bộ</a>
				<a href="http://pluginmcvn.cf/resource/free.plugin?category=1&page=<?php echo $page; ?>">Công cụ và tiện ích</a>
				<a href="http://pluginmcvn.cf/resource/free.plugin?category=2&page=<?php echo $page; ?>">Vui vẻ</a>
				<a href="http://pluginmcvn.cf/resource/free.plugin?category=3&page=<?php echo $page; ?>">Quản lý world</a>
				<a href="http://pluginmcvn.cf/resource/free.plugin?category=4&page=<?php echo $page; ?>">Kinh tế</a>
				<a href="http://pluginmcvn.cf/resource/free.plugin?category=5&page=<?php echo $page; ?>">Chế độ chơi</a>
				<a href="http://pluginmcvn.cf/resource/free.plugin?category=6&page=<?php echo $page; ?>">Thư viện & API</a>
			</div>
		</div>
