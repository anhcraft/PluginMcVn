...
if ($res->num_rows > 0):
	while($aif = $res->fetch_assoc()) {
		$s += 1;
		$total += 1;
		if(($category != null && $aif["category"] != $category) || $aif["pre"] == NULL){
			$s -= 1;
			continue;
		}
		if($pbegin < $s && $s <= $pend){
...
<div class="list-plugin-sub">
	...
	<div id="list-plugin-sub-content">
		...
		<div id="list-plugin-sub-info">
			...
			<span><a href="http://pluginmcvn.cf/resource/premium.plugin?category='.$aif["category"].'">'.toCategory($aif["category"]).'</a></span>
		</div>
		<div id="list-plugin-sub-description">'.$aif["description"].'</div>
		<div id="list-plugin-sub-price">Giá: '.$aif["price"].'</div>
	</div>
</div>		
			';
		} else {
...
if($listPlugin == ""){
	if(1 < ((int) $page)){
		if($category == null){
			header("Location: ". regurl("@p://@d/resource/premium.plugin?page=".(((int) $page)-1)));
		} else {
			header("Location: ". regurl("@p://@d/resource/premium.plugin?page=".(((int) $page)-1)."&category=".$category));
		}
	} else {
		header("Location: ". regurl("@p://@d/resource/premium.plugin"));
	}
}
...
if(1 < $rs){
	...
	for($i = 0; $i < $rs; $i++){
		...
		if($i == 0){
			$pagemenu .= '<li '.$pms.'><a href="http://pluginmcvn.cf/resource/premium.plugin">'.($i+1).'</a></li>';
			continue;
		}
		$pagemenu .= '<li '.$pms.'><a href="http://pluginmcvn.cf/resource/premium.plugin?page='.($i+1).'">'.($i+1).'</a></li>';
	}
...
<div class="container pluginmcvn-container">
	<h1 id="pluginmcvn-bigtitle" class="padd-big">Plugin Premium</h1>
	<div id="side-area">
		<div id="side-top" class="padd-large">
			<div class="side-top-heading">Danh mục</div>
			<div class="side-top-menu">
				<a href="http://pluginmcvn.cf/resource/premium.plugin?page=<?php echo $page; ?>">Toàn bộ</a>
				<a href="http://pluginmcvn.cf/resource/premium.plugin?category=1&page=<?php echo $page; ?>">Công cụ và tiện ích</a>
				<a href="http://pluginmcvn.cf/resource/premium.plugin?category=2&page=<?php echo $page; ?>">Vui vẻ</a>
				<a href="http://pluginmcvn.cf/resource/premium.plugin?category=3&page=<?php echo $page; ?>">Quản lý world</a>
				<a href="http://pluginmcvn.cf/resource/premium.plugin?category=4&page=<?php echo $page; ?>">Kinh tế</a>
				<a href="http://pluginmcvn.cf/resource/premium.plugin?category=5&page=<?php echo $page; ?>">Chế độ chơi</a>
				<a href="http://pluginmcvn.cf/resource/premium.plugin?category=6&page=<?php echo $page; ?>">Thư viện & API</a>
			</div>
		</div>
